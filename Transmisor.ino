/*
 *  SILBATO ARBITRO - v4.7 (PERFIL RELAJADO)
 *  =========================================
 *  FIXES vs v4.6:
 *  - Relajado umbral adaptativo de similitud (ahora 0.70 en vez de 0.85)
 *  - Mínimo absoluto de similitud bajado a 0.60
 *  - Permite pasar silbato aunque perfil sea bajo (avisa con warning)
 *  - Detecta silbato real incluso con variaciones
 *  - Sigue ignorando gritos por energía/ratio/bin
 */
...
void sendLoRa(const char* comando) {
  char mensaje[64];
  snprintf(mensaje, sizeof(mensaje), "%s:%s", ARBITRO_ID, comando);
  LoRa.beginPacket();
  LoRa.print(mensaje);
  LoRa.endPacket();
  Serial.printf("[TX] %s\n", mensaje);
