/*
 *  RECEPTOR SILBATO ARBITRO - v3.0 WebServer + LoRa (Modo AP)
...
function applyBattery(id, pct) {
  ...
  if (!container) return;
  if (pct < 0) { container.innerHTML = "<span class=\"battery-unknown\">No data</span>"; return; }
  var barClass = "battery-bar";
  if (pct <= 20) barClass += " low";
  else if (pct <= 50) barClass += " medium";
  container.innerHTML =
    "<span class=\"battery-label\">Battery</span>" +
    "<div class=\"battery-bar-wrap\"><div class=\"" + barClass + "\" style=\"width:" + pct + "%\"></div></div>" +
...
function showValModal(results) {
  ...
  for (var i = 0; i < 3; i++) {
    ...
    var bat = r.pct >= 0 ? " - " + r.pct + "%" : "";
    html += "<div class=\"val-row\"><span class=\"val-name\">" + NAMES[i] + bat + "<​/span>" + badge + "<​/div>";
  }
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println(F("\n=== RECEPTOR v3.0 WebServer + LoRa (Modo AP) ==="));
...
void checkLoRa() {
  ...
  String comando = pkt.substring(sep + 1);
